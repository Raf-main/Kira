    ...
    ) : IAccountService
    {
        ...
        public async Task RegisterAsync(RegistrationRequest registrationRequest)
        {
            ...
            if (!result.Succeeded)
            {
                ...
                throw new ValidationException($"Something went wrong while creating user with email {registrationRequest.Email}", errors);
            }
        }
        ...
        public async Task<RefreshTokenResponse> RefreshTokenAsync(RefreshTokenRequest refreshRequest)
        {
            ...
            return new RefreshTokenResponse(newAccessToken, newRefreshToken.Token, newRefreshToken.ExpirationTime, GetUserResponse(user));
        }

        private async Task<IEnumerable<Claim>> GetClaimsAsync(User user)
        {
            var authClaims = new List<Claim> { new(ClaimTypes.Email, user.Email!), new(ClaimTypes.NameIdentifier, user.Id) };

            var userRoles = await userManager.GetRolesAsync(user);

            if (userRoles is { Count: > 0 })
            {
                authClaims.AddRange(userRoles.Where(role => !string.IsNullOrEmpty(role)).Select(role => new Claim(ClaimTypes.Role, role)));
            }
